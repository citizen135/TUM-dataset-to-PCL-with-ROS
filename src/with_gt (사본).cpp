...
int main(int argc, char **argv){
  ...
  pcl::PointCloud<pcl::PointXYZRGB> mapCloud;
  ...
  while(file.good()) //eof, bad, fail 함수가 거짓의 참을 반환할 때까지..
    ...
    for ( int v=0; v<rgb.rows; v++ )
        for (int u=0; u<rgb.cols; u++)
        {
            ...
            // pcl_ros::transformPointCloud(pointCloud, mapCloud, transformStamped);
            ...
            // cout<<&pointCloud.points<<endl;
            // cout<<&ptr_transformed->points<<endl;
        }
