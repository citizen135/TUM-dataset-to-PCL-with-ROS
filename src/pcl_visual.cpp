...
int main(int argc, char **argv){
  ...
  ros::init(argc,argv, "pcl_visual");
  ros::NodeHandle nh;       // NodeHandle 선언

  ros::Publisher pub2= nh.advertise<pcl::PointCloud<pcl::PointXYZRGB>> ("current_points",2);      // Publisher 선언
  ros::Publisher pub3= nh.advertise<pcl::PointCloud<pcl::PointXYZRGB>> ("all_points",2);      // Publisher 선언
  ...
  while(file.good()) //eof, bad, fail 함수가 거짓의 참을 반환할 때까지..
    ...
    vector<float> quato { stof(row[5]), stof(row[6]), stof(row[7]), stof(row[8])};
    double inner;
